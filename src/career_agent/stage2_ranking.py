...
def _find_source_job(ranked: dict, jobs: list[dict]) -> dict:
    if ranked.get("record_id"):
        return next((job for job in jobs if job.get("record_id") == ranked["record_id"]), {})
    ...
    *,
    ...
    for index, (ranked, job) in enumerate(zip(selected, source_jobs)):
        ...
        if assessment is None:
            ...
            if evidence_level != "full_jd" and confidence == "high":
                confidence = "medium"
            if evidence_level == "source_only" and not job.get("source_evidence"):
                confidence = "low"
        ...
        results.append(
            Stage2RankedJob(
                record_id=str(ranked.get("record_id") or job.get("record_id") or ""),
                candidate_index=index,
