...
def rank_job(student_profile: dict, job: dict) -> Stage1RankedJob:
    ...
    # A tiny breadth bonus breaks ties in favour of matches supported by several
    # independent skills without allowing breadth to dominate true coverage.
    breadth = len(matched_resume) + 0.75 * len(matched_course)
    breadth_bonus = min(8.0, breadth * 1.5)
    score = min(100.0, round(coverage * 92.0 + breadth_bonus, 1))
