...
def _seed_direct_url_from_company_hosts(
    ...
    for known_host in official_hosts[:2]:
        try:
            results = search_public_web(
                f'site:{known_host} "{signal.role_title}" {location}',
                max_results=6,
            )
            calls += 1
            ...
            calls += 1
            warnings.append(
                f"company-session search failed for {known_host}: {type(exc).__name__}: {exc}"
            )
            continue
        for result in results:
            try:
                ...
                continue
            if not _host_matches(candidate_host, known_host):
                continue
...
def _official_hosts_from_package(package, company: str | None) -> list[str]:
    hosts: list[str] = []
    if is_plausible_official_url(package.official_job_url, company):
        try:
            value = urlparse(package.official_job_url).netloc.lower()
            if value:
                hosts.append(value)
        except ValueError:
            pass
    for candidate in package.candidates:
        if candidate.tier != "official":
            continue
        if not is_plausible_official_url(candidate.url, company):
            continue
        if candidate.host:
...
def _usable_page_text(page_text: str, title: str | None) -> tuple[str | None, str | None]:
    ...
    if not _useful_jd_text(cleaned, title):
        return None, "shell/insufficient JD text"
    return cleaned[:MAX_JD_TEXT_CHARS], None
...
def _fetch_best_jd(package, source_context: str) -> tuple[str, str | None, str, list[str], int]:
    ...
    company = package.identity.company
...
def _result_relevant(signal: OpportunitySignal, result: SearchResult) -> bool:
    ...
    company_tokens = [
        token
        for token in TOKEN_PATTERN.findall((signal.company or "").lower())
        if len(token) >= 4
    ]
...
def _simple_web_fallback(
    ...
    location = signal.location or "Singapore"
    query = f'"{company}" "{title}" {location} job careers'.strip()
    search_calls = 0
    fetch_calls = 0
    try:
    ...
    primary_candidates = [
        result for result in relevant if is_plausible_official_url(result.url, signal.company)
    ]
    secondary_candidates = [result for result in relevant if is_secondary_url(result.url)]
    ...
    for kind, candidates in (
        ("fetched_official", primary_candidates),
        ("fetched_secondary", secondary_candidates),
    ):
        for result in candidates[:4]:
            try:
                ...
                continue
            cleaned, reason = _usable_page_text(page.text, signal.role_title)
            if cleaned:
                return (
                    primary_url,
                    secondary_url,
    ...
    for company_items in groups.values():
        ...
        for index, original_signal in company_items:
            ...
            primary_url = (
                ...
                if is_plausible_official_url(package.official_job_url, signal.company)
                else None
            )
            secondary_url = next(
                (
